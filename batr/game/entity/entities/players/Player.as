package batr.game.entity.entities.players
{
	...
	public class Player extends EntityCommon implements IPlayerProfile
	{
		...
		//Respawn
		public var respawnTick:int=-1//negative number means isn't respawning
		...
		{
			if(value==this._weaponUsingCD) return
		...
		{
			if(value==this._weaponChargeTime) return
		...
		{
			if(value==this._weaponChargeMaxTime) return
		...
		{
			if(this._weapon==null) return false
		...
		{
			if(!this.weaponNeedsCD) return 1
		...
		public function set health(value:uint):void
		{
			...
			if(this._GUI!=null) this._GUI.updateHealth()
		}
		...
		{
			if(value==this._maxHealth) return
			this._maxHealth=value
			if(value<this._health) this._health=value
		...
		{
			if(value==this._infinityLife) return
		...
		{
			if(value==this._customName) return
		...
		public function set pressUse(turn:Boolean):void
		{
			if(this.isPress_Use&&!turn)
			{
				...
				if(isCharging) this.onDisableCharge()
				return
			}
			this.isPress_Use=turn
		}
		...
		public function initVariablesByRule(weaponID:int,uniformWeapon:WeaponType=null):void
		{
			...
			else this._weapon=WeaponType.fromWeaponID(weaponID)
		}
		...
		protected function onDeath(damage:uint,attacker:Player=null):void
		{
			...
			if(attacker!=null) attacker.onKillPlayer(this,damage);
		}
		
		protected function onKillPlayer(victim:Player,damage:uint):void
		{
			if(victim!=this&&!this.isRespawning) this.experience++;
		}
		...
		public function dealRespawn():void
		{
			...
			{
				this.respawnTick=-1;
				if(!this._infinityLife&&this._lifes>0) this._lifes--;
		...
		public function resetCharge(includeMaxTime:Boolean=true,updateGUI:Boolean=true):void
		{
			...
			if(includeMaxTime) this._weaponChargeMaxTime=0;
			if(updateGUI) this._GUI.updateCharge();
		}
		...
		{
			if(attacker==null) return attackerWeapon==null?0:attackerWeapon.defaultDamage;
			if(attackerWeapon==null) attackerWeapon=attacker.weapon;
			if(attackerWeapon!=null) return attackerWeapon.getBuffedDamage(defaultDamage,attacker.buffDamage,this.buffResistance);
			return 0;
		}
		...
		{
			return defaultRadius*this._buffRadius/(1+this._buffRadius)*4;
		}
		...
		{
			//Deal fillColor
			if(isNaN(fillColor)) this._fillColor=this._team.defaultColor;
		...
		public function setCarriedBlock(block:BlockCommon,copyBlock:Boolean=true):void
		{
			...
			{
				if(this._carriedBlock!=null&&this.contains(this._carriedBlock)) this.removeChild(this._carriedBlock);
		...
		public function useWeapon():void
		{
			...
			if(this.weaponNeedsCharge) this._GUI.updateCharge();
		}
	}
}
