package batr.common
{
	...
	public class Color
	{
		...
		{
			if(RGB==null||
			   RGB.length!=3) return defaultHEX;
		...
		public static function HEXtoRGB(I:uint):Vector.<Number>
		{
			...
			var Gr:Number=snapRGB((I&0x00ff00)>>8);
			var Bl:Number=snapRGB(I&0x0000ff);
			returnVec[0]=Re;
			returnVec[1]=Gr;
			returnVec[2]=Bl;
			return returnVec;
		...
		{
			if(RGB==null||
				RGB.length!=3) return defaultHSV;
			var R:Number=RGB[0];
			var G:Number=RGB[1];
			var B:Number=RGB[2];
		...
		{
			if(HSV==null||
			   HSV.length!=3) return defaultRGB;
			var H:Number=HSV[0];
			var S:Number=HSV[1];
			var V:Number=HSV[2];
